...
class CardForm extends Component {
    ...
    validateField = (fieldname, value) => {
        ...
        let expdatevalid = this.state.expMvalid;
        ...
        switch (fieldname) {
            ...
            case('cvc'): {
                ...
                formErrorField = cvcvalid?'':'Wrong CVC format, must be 3 digits number. '
            }
            default: {
                if(value.length===0){
                ...
                else if((fieldname==='expM' && value>12) || (fieldname==='expY' && value<22)) {
    ...
    validateForm() {
        this.setState({formValid: this.state.cardnamevalid && this.state.cardnumbervalid && this.state.expdatevalid  && this.state.cvcvalid });
      }
    ...
    render () {
        return (
            <form class="w-full max-w-[420px]">
                ...
                <div>
                    <button type="submit" className="w-full text-center text-lg bg-[#21092F] rounded-md p-3 text-gray-200 font-grotesk" disabled={!this.state.formvalid}  >Confirm</button>
                </div>
            </form>
        )
    }
